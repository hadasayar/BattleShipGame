...
shell
Copy code
git clone https://github.com/yourusername/battleship-game-api.git
...
Each player has a board with a grid of cells.
Players take turns guessing the coordinates to locate and sink the opponent's ships.
Ships are placed randomly on the grid and can be of different sizes (battleship, cruiser, destroyer).
...
CellState (Enum) - Defines the state of a cell on the board (empty, ship, hit, miss).
Game - Represents a Battleship game with players and their boards.
Player - Represents a player in the game with a name and a board.
BattleShip - Represents a ship on the board.
Service
...
Use your favorite API client (Postman, Swagger) to interact with the API endpoints.
